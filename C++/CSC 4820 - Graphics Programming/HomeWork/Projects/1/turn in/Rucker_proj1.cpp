...
// This program only loads the 3d model. Does not move or rotate camera or move 
// object
//=============================================================================
...
static const std::string modelFile = "square_high.obj";
...
// Camera Coordinates
float alpha = 1.0f, beta = 1.0f, r = 5.0f;
...
bool ImportFrom3DFile(const std::string& pFile)
{
	...
	modelWindowSize = 8.f / temp; // Model zoom percentage
...
void scene_Render(void)
{
	...
	// set camera matrix
	setViewCamera(cameraX, cameraY, cameraZ, 0, 0, 0);
...
// Shader Info
//============================================================
void programLogDisplay(GLuint object)
{
	int log = 0, characters = 0;
	char *infoLog;

	glGetProgramiv(object, GL_INFO_LOG_LENGTH, &log);

	if (log > 0)
	{
		infoLog = (char *)malloc(log);
		glGetProgramInfoLog(object, log, &characters, infoLog);
		printf("%s\n", infoLog);
		free(infoLog);
	}
}
