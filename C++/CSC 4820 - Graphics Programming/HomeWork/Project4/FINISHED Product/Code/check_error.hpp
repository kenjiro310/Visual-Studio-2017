...
void printAiSceneInfo(const aiScene* scene, AiScenePrintOption option) {
	...
	for (unsigned int i = 0; i < scene->mNumMeshes; i++) {
		...
		cout << endl << "---------- Mesh #" << i << " " << currentMesh->mName.C_Str() 
			<< "----------" << endl;
		...
		unsigned int currentMaterialIndex = currentMesh->mMaterialIndex;
		cout << "---------- Material Property for Mesh #" << i << " ----------" << endl; 
		cout << "Material index " << currentMaterialIndex << endl;
		
		aiMaterial* currentMaterial = scene->mMaterials[currentMaterialIndex];
		...
		for (int k = 0; k < diffuseTextureCount; k++) {
			aiString textureFilePath;
			if(AI_SUCCESS == scene->mMaterials[currentMaterialIndex]->GetTexture(aiTextureType_DIFFUSE, k, &textureFilePath)) {
